...
def klines_binance(symbol, interval, limit=200):
    try:
        ...
        return _norm([[k[1],k[2],k[3],k[4]] for k in r.json()])
    except Exception:
        return []
...
def klines_bybit(symbol, interval, limit=200):
    try:
        ...
        return _norm([[k[1],k[2],k[3],k[4]] for k in rows])
    except Exception:
        return []
...
def calc_squeeze(klines, bb_len=20, bb_mult=2.0, kc_len=20, kc_mult=1.5, clust_pct=0.03):
    """
    ...
    # ── helper: vol sqz state for an arbitrary slice ──
    def vol_state(c, h, l):
        t = tr_series(h, l, c)
        bb_b = sma(c, bb_len)
        bb_d = stdev(c, bb_len)
        kc_b = ema(c, kc_len)          # EMA basis — KEY FIX
        kc_a = rma(t, kc_len)          # RMA ATR   — KEY FIX
        ...
        return bb_u < kc_u and bb_l > kc_l   # True = squeezed

    # ── helper: SMA sqz state ──
    def sma_state(c):
    ...
    cur_vol  = vol_state(closes, highs, lows)
    prev_vol = vol_state(closes[:-1], highs[:-1], lows[:-1])

    cur_sma,  cur_rng  = sma_state(closes)
    prev_sma, _        = sma_state(closes[:-1])

    vol_sqz = "on"   if cur_vol  else ("fire" if prev_vol  else "none")
    sma_sqz = "on"   if cur_sma  else ("fire" if prev_sma  else "none")

    # ── Count consecutive squeeze bars (lookback up to 50) ──
    sqz_bars = 0
    for i in range(len(closes) - 2, max(len(closes) - 52, 9), -1):
        c_sl = closes[:i+1]
        h_sl = highs[:i+1]
        l_sl = lows[:i+1]
        v = vol_state(c_sl, h_sl, l_sl)
        s, _ = sma_state(c_sl)
        if v or s:
            sqz_bars += 1
        else:
    ...
    # ── Elephant / tail bar metrics ──
    last_body  = abs(closes[-1] - opens[-1])
    last_range = highs[-1] - lows[-1]
    last_wick  = (last_range - last_body) / last_range if last_range > 0 else 0.0
    ...
    last_bar_mult = last_body / sqz_avg if sqz_avg > 0 else 0.0
...
def scan(tf):
    ...
    for sym in PAIRS:
        ...
        klines, ksrc = get_klines(sym, interval)
        sqz = calc_squeeze(klines) if klines else {
...
<div class="settings-drawer" id="settingsDrawer">
  ...
  <div class="settings-grid">
    <div class="set-card"><div class="set-lbl">CLUSTER THRESHOLD %</div><div class="set-row"><input type="range" min="5" max="80" value="30" step="1" oninput="sv('clustV',(this.value/10).toFixed(1)+'%')"><span class="set-val" id="clustV">3.0%</span></div></div>
...
function render(){
  ...
  st.data=st.data.map(r=>{
    const isElephant=(r.volSqz==="fire"||r.smaSqz==="fire")&&r.lastBarMult>=s.elephMult;
    const isTail=isElephant&&r.lastWickRatio>=s.tailRatio;
    return{...r,isElephant,isTail,confirmType:isTail?"tail":isElephant?"elephant":null};
  });
  let rows=[...st.data];
...
function openModal(sym){
  ...
  if(r.confirmType){
    ...
    confirmSection=`<div class="modal-section"><div class="modal-section-title">BREAKOUT CONFIRMATION</div><div class="confirm-box" style="border-color:${color}"><div class="confirm-title" style="color:${color}">${type} DETECTED &mdash; ENTRY VALID</div><div class="confirm-row"><span class="confirm-lbl">Bar size vs SQZ avg</span><span class="confirm-val" style="color:${color}">${r.lastBarMult.toFixed(2)}x</span></div><div class="confirm-row"><span class="confirm-lbl">Required mult</span><span class="confirm-val">${s.elephMult.toFixed(1)}x</span></div>${r.isTail?`<div class="confirm-row"><span class="confirm-lbl">Wick ratio</span><span class="confirm-val">${(r.lastWickRatio*100).toFixed(0)}% (min ${(s.tailRatio*100).toFixed(0)}%)</span></div>`:""}<div class="confirm-row"><span class="confirm-lbl">SQZ candles held</span><span class="confirm-val">${r.sqzBarsCount}</span></div></div></div>`;
  } else if(r.volSqz!=="none"||r.smaSqz!=="none"){
    confirmSection=`<div class="modal-section"><div class="modal-section-title">BREAKOUT CONFIRMATION</div><div class="confirm-box"><div class="confirm-title" style="color:var(--muted)">WAITING FOR ELEPHANT / TAIL BAR</div><div class="confirm-row"><span class="confirm-lbl">SQZ candles so far</span><span class="confirm-val">${r.sqzBarsCount}</span></div><div class="confirm-row"><span class="confirm-lbl">Need bar size &ge;</span><span class="confirm-val">${s.elephMult.toFixed(1)}x avg SQZ candle</span></div><div class="confirm-row"><span class="confirm-lbl">Tail wick threshold</span><span class="confirm-val">${(s.tailRatio*100).toFixed(0)}% of bar</span></div></div></div>`;
  }

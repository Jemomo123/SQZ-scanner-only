...
@app.route('/debug/<sym>/<tf>')
def debug(sym, tf):
    interval = TF_MAP.get(tf, "15m")
    prices = get_prices()
    live = prices.get(sym)
    if not live:
        return jsonify({"error": "no price for " + sym})
    closes = get_klines(sym, interval)
    if len(closes) < 100:
        return jsonify({"error": "only " + str(len(closes)) + " candles returned"})
    closes_with_live = closes + [live]
    s20  = sma(closes_with_live, 20)
    s100 = sma(closes_with_live, 100)
    price = closes_with_live[-1]
    g1 = abs(price - s20)  / price * 100
    g2 = abs(price - s100) / price * 100
    g3 = abs(s20   - s100) / price * 100
